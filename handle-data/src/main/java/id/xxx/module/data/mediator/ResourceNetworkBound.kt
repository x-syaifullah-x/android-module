...
class ResourceNetworkBound<ResultType, RequestType> private constructor(
    ...
    private val blockFetch: suspend () -> Flow<NetworkResponse<RequestType>>?,
    private val blockOnFetch: suspend (NetworkResponse<RequestType>, ResultType?) -> Unit,
    ...
    companion object {
        fun <ResultType, RequestType> create(
            ...
            /**
             * @if [blockOnResult] is false [blockResult] always [ResourceSealed.Empty]
             */
             ...
             */
            blockFetch: suspend () -> Flow<NetworkResponse<RequestType>>? = { null },
            /**
             * called after [blockFetch]
             */
            blockOnFetch: suspend (NetworkResponse<RequestType>, ResultType?) -> Unit = { _, _ -> },
    ...
    override suspend fun onFetchSuccess(fetchType: RequestType) {
        blockOnFetch(NetworkResponse.Success(fetchType), result()?.firstOrNull())
    }

    override suspend fun <T : Throwable> onFetchError(err: T, resultType: ResultType?) {
        blockOnFetch(NetworkResponse.Error(err = err), resultType)
    }
    ...
    private suspend fun getResource(
        ...
        isLoading -> ResourceSealed.Loading
        throwable != null -> {
            when (throwable) {
                is ResourceNetworkBoundError -> throw throwable
                else -> ResourceSealed.Error(data = resultType, error = throwable)
            }
        }
        resultType != null && onResult(resultType) -> ResourceSealed.Success(resultType)
        else -> ResourceSealed.Empty
    }

    private suspend fun getResult(
        err: Throwable? = null, loading: Boolean = false
    ): Flow<ResourceSealed<ResultType>> = result()?.run {
    ...
    override fun asFlow() = flow {
        ...
        val fetch = fetch()?.flatMapLatest { response ->
            when (response) {
                is NetworkResponse.Success -> onFetchSuccess(response.data)
                    .run { getResult() }
                is NetworkResponse.Error -> onFetchError(response.err, result()?.firstOrNull())
                    .run { getResult(err = response.err) }
            }
